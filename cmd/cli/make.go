...
func doMake(arg2, arg3 string) error {
	...
	case "migration":
		...
		upFile := ras.RootPath + "/migrations/" + fileName + "." + dbType + ".up.sql"
		downFile := ras.RootPath + "/migrations/" + fileName + "." + dbType + ".down.sql"
	...
	case "handler":
		if arg3 == "" {
			exitGracefully(errors.New("you must give a handler name"))
		}

		fileName := ras.RootPath + "/handlers/" + strings.ToLower(arg3) + ".go"
	...
	case "model":
		...
		fileName := ras.RootPath + "/data/" + strings.ToLower(modelName) + ".go"
		if fileExists(fileName) {
			exitGracefully(errors.New(fileName + " already exists"))
		}
	...
	case "mail":
		...
		htmlMail := ras.RootPath + "/mail/" + strings.ToLower(arg3) + ".html.tmpl"
		plainMail := ras.RootPath + "/mail/" + strings.ToLower(arg3) + ".plain.tmpl"
